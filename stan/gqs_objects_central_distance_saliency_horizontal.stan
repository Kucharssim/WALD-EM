...
parameters{
  ...
  simplex[5] weights; // objects, saliency, central bias, distance, horizontal bias
...
generated quantities{
  ...
  for(i in 1:N_obs){
    ...
    } else if(factor == 4){ // central bias
      x_rep[i] = trunc_normal_rng(400, sigma_center, 0, 800);
      y_rep[i] = trunc_normal_rng(300, sigma_center, 0, 600);
    } else{ // horiontal bias
      if(current_order == 1){
        xy_rep = direction_bias_rng(rep_vector(0.5, 2), to_vector([0, pi()]), rep_vector(kappa, 2), 400, 300, 0, 800, 0, 600);
      } else{
        xy_rep = direction_bias_rng(rep_vector(0.5, 2), to_vector([0, pi()]), rep_vector(kappa, 2), x[i-1], y[i-1], 0, 800, 0, 600);
      }
      x_rep[i] = xy_rep[1];
      y_rep[i] = xy_rep[2];
    }
