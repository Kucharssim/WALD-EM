...
The following checks that the density function of the von Mises mixture is implemented correctly. Here, we simulate data from two mixture components with means fixed to 0 and $\pi$, equal concentrations $\kappa = 8$, and equal mixture weights.
...
Below we fit the model to see whether we recover the parameter $\kappa$:
```{r echo=TRUE}
