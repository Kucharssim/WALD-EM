...
sim_data_list <- lapply(seq_along(sim_parameters_list), function(i) {
  ...
  d <- simulate(t_max = 10, pi = pars$pi, delta = pars$delta, alpha = pars$alpha, sigma_attention = pars$sigma_attention, n_max = 100)
  d$sim <- i
  return(d)
  })
...
plot(sim_data$x, sim_data$y, pch = 19, cex = 0.1)
```
