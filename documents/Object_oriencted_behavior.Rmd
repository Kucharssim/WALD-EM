...
simulate <- function(t_max, pi, delta, alpha, sigma_attention, n_max = t_max * 10){
  ...
  while(t <= t_max && length(x) < n_max){
    ...
    x_now <- rnorm(1, objects$x[which_factor], sigma_x[which_factor])#, dim_img$min_x, dim_img$max_x)
    y_now <- rnorm(1, objects$y[which_factor], sigma_y[which_factor])#, dim_img$min_y, dim_img$max_y)
...
# fits <- lapply(sim_data_list, function(d){
#   pb$tick()$print()
#   stan_data <- list(N_obs = nrow(d), x = d$x, y = d$y, duration = d$duration, 
#                     N_objects = n_objects, objects_center_x = objects$x, objects_center_y = objects$y,
#                     objects_width = objects$width, objects_height = objects$height)
#   
#   fit <- rstan::sampling(model, stan_data, chains  = chains, cores = cores, iter = 510, warmup = 500, refresh = 0)
#   return(fit)
# })
```

```{r sbc}
sbc <- lapply(c("alpha", "delta", "sigma_attention"), function(p) fits2rank(fits, sim_parameters[[p]], p))
```
