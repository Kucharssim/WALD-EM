...
And below are the summaries of the distributions of fixation
durations
...
We fit the model on all 1000 simulated datasets and computed the rank
based statistics from 10 MCMC samples (after thinning by a factor 2).

    ## Stan model 'object_oriented_behavior' does not contain samples.
    ## Stan model 'object_oriented_behavior' does not contain samples.
    ## Stan model 'object_oriented_behavior' does not contain samples.
    ## Stan model 'object_oriented_behavior' does not contain samples.
    ## Stan model 'object_oriented_behavior' does not contain samples.
...
![](object_oriented_behavior_files/figure-gfm/sbc_result-1.png)<!-- -->

Above are the rank statistics based on 10 draws from the posterior for
parameters alpha, delta, sigma\_attention. The barplots seem relatively
uniformly distributed, which does not suggest strong bias and/or issue
with too certain/uncertain estimates. The histograms of the priors (red)
and data averaged posteriors (green) also overlap sugesting that the
correct posteriors are estimated.
